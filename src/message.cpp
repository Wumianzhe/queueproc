...
Message parse(const std::string &line) {
  // requiring 2 digits means leading zeroes are necessary, incoming messages
  // have id of 1-4 body is anything non-empty for now and will be looked at
  // during processing as contents depends on id
  ...
  std::chrono::minutes time = std::chrono::hours(tm_time.tm_hour) +
                              std::chrono::minutes(tm_time.tm_min);
